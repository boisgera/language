...
def build_corpus(root_dir):
    ...
    print(f"Corpus size: {s / (1024 ** 3):.2f} GiB")
    return d

SEP = np.uint16([2**8]) # start/end of document separator token.
# Corresponds to 

def build_array(root_dir):
    ...
    print(f"Big array size: {big_array.nbytes / (1024 ** 3):.2f} GiB")
    return big_array
...
if __name__ == "__main__":
    ...
    output = open("digrams.py", "w", encoding="utf-8")
    try:
        output.write("# This file is generated by chars.py. Do not edit.\n")
        output.write("digrams = {\n")
        for i in tqdm.tqdm(range(2**14 - 256)):
            digram_counts = count_digrams_perf(corpus)
            new_digram, count = digram_counts[0]
            print(f"Top digram: {new_digram}: {count} count")
            
            # Update the token dictionary
            new_token = len(token_to_bytes)
            digrams[new_digram] = new_token
            new_bytes = token_to_bytes[new_digram[0]] + token_to_bytes[new_digram[1]]
            output.write(f"    {new_digram}: {new_token},  # {new_bytes} ({count}) \n")
